<?php
if (!defined('_JSB_')) exit; // 개별 페이지 접근 불가
...
function editor_html($id, $content, $is_dhtml_editor=true)
{
    ...
    if( $is_dhtml_editor && $content ){       //글쓰기 기본 내용 처리
        if( preg_match('/\r|\n/', $content) && $content === strip_tags($content, '<a><strong><b>') ) {  //textarea로 작성되고, html 내용이 없다면
            $content = nl2br($content);
        }
    }

    $editor_url = HOME_URL.'/vendor/smarteditor2';
    ...
    $html .= "\n<textarea id=\"$id\" name=\"$id\" class=\"$smarteditor_class\" maxlength=\"65536\" style=\"width:100%;height:600px\">$content</textarea>";
...
if (!defined('FT_NONCE_UNIQUE_KEY'))
    define( 'FT_NONCE_UNIQUE_KEY' , sha1($_SERVER['SERVER_SOFTWARE'].session_id()) );
