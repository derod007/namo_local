...
function get_regioncode_select($name, $selected='', $event='')
{
    ...
    //$sql = " select * from {$jsb['regioncode_table']} where right(code,5) = '00000' and left(code,5) = region and use_yn = '존재'  ";
    $sql = " select * from {$jsb['regioncode_table']} where step='2' ";
...
function get_sidocode_select($name, $selected='', $event='')
{
    ...
    //$sql = " select * from {$jsb['regioncode_table']} where right(code,5) = '00000' and left(code,5) = region and use_yn = '존재'  ";
    $sql = " select * from {$jsb['regioncode_table']} where step='1' ";
    $result = sql_query($sql);
    $str = "<select id=\"$name\" name=\"$name\" $event class=\"form-control\">\n";
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        if ($i == 0) $str .= "<option value=\"\">선택</option>";
...
// 지역코드 정보를 얻는다.
function get_dongcode_select($name, $selected='', $event='')
{
    global $jsb;
	
    //$sql = " select * from {$jsb['regioncode_table']} where right(code,5) = '00000' and left(code,5) = region and use_yn = '존재'  ";
    $sql = " select * from {$jsb['regioncode_table']} where step='0' ";
    $result = sql_query($sql);
    $str = "<select id=\"$name\" name=\"$name\" $event class=\"form-control\">\n";
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        if ($i == 0) $str .= "<option value=\"\">선택</option>";
        $str .= option_selected($row['code'], $selected, "(".$row['code'].") ".$row['dong']);
    }
    $str .= "</select>";
    return $str;
...
// 지역코드 정보를 얻는다.
function get_regioncode2nd_select($name, $selected='', $sido='', $event='')
{
    global $jsb;
	$whereqry = '';
	if($sido) {
		$sido = substr($sido,0,2);
		$whereqry = " and region like '{$sido}%' ";
	}
    //$sql = " select * from {$jsb['regioncode_table']} where right(code,5) = '00000' and left(code,5) = region and use_yn = '존재'  ";
    $sql = " select * from {$jsb['regioncode_table']} where step='2' {$whereqry}";
    $result = sql_query($sql);
    $str = "<select id=\"$name\" name=\"$name\" $event class=\"form-control\">\n";
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        if ($i == 0) $str .= "<option value=\"\">선택</option>";
