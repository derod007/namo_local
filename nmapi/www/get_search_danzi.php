...
if($searchtxt!='') {
	if(!strpos($searchtxt,' ')) {		// 검색어에 공백문자가 포함되지 않았으면
		$where[] = " (danzi like '%$searchtxt%' or dong like '%$searchtxt%') ";
	} else {
		$where[] = " (danzi like '%$searchtxt%' or concat(dong, ' ', zibun) like '%$searchtxt%') ";
	}
}
