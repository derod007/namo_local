...
include_once($_SERVER['DOCUMENT_ROOT'].'/inc/dbconfig.php');    // 설정 파일
...
// 공통
//------------------------------------------------------------------------------
    include_once(JSB_PATH.'/inc/common.lib.php');    // 공통 라이브러리
...
if(!is_dir(JSB_SESSION_PATH)) {
	@mkdir(JSB_SESSION_PATH, DIR_PERMISSION);
	@chmod(JSB_SESSION_PATH, DIR_PERMISSION);
}

session_save_path(JSB_SESSION_PATH);
...
if(isset($_SESSION["ss_login_id"])) {
	$member = sql_fetch("select idx, mb_id, mb_name, mb_level, mb_bizname, mb_joindate, mb_use, mb_display, is_sub, parent_id from {$jsb['member_table']} where mb_id='{$_SESSION['ss_login_id']}'");
}
