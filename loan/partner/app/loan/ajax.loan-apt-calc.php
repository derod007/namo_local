...
if($wr_live == 'O' || $wr_live == 'F') $wr_deposit = 0;	// 본인거주/무상거주는 보증금 0원으로
...
if($judge_price) {
	...
	2024-06-10 항목별로 동일차주 여부 적용
	$mortgage['total'] = $total_amt;
	$mortgage['same'] = $same_amt;
	$mortgage['differ'] = $differ_amt;
	
	2024-06-27
	1. 한도 3천 미만은 한도부족으로 표시되게
	2. 본인/무상거주인 경우 소액보증금 공제
	3. 소액임차보증금 공제시 기준금액은 상관없이 소액임차보증금 기준으로 차감
	...
	**/
	...
	// 2024-06-10 선순위 각각에 대한 지분율 적용
	$wr_mortgage_sub_same = $mortgage['same'];
	$wr_mortgage_sub_differ = $mortgage['differ'] * $calc_part_percent;
	$wr_mortgage_sub_total = $wr_mortgage_sub_same + $wr_mortgage_sub_differ;
	
	$title_mort = "";
	if($differ_amt > 0) {
		$title_mort = "(지분)";
		$judge_price4 = $judge_price3 - $wr_mortgage_sub_total;
	} else {
		$title_mort = "(전체)";
		$judge_price4 = $judge_price3 - $wr_mortgage_sub_total;
	}
...
$sql = " update `loan_apt_tmp`
			set wr_part_percent = '{$wr_part_percent}',
				wr_judge   = '{$wr_judge}',
